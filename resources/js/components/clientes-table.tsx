...
export default function ClientesTable({ clientes, onEdit, onDelete }: Props) {
    ...
    const columnHelper = useMemo(() => createColumnHelper<Cliente>(), []);

    const columns = useMemo(
        () => [
            columnHelper.accessor('nombre', { header: 'Nombre' }),
            columnHelper.accessor('apellido', { header: 'Apellido' }),
            columnHelper.accessor('n_documento', {
                header: 'N.º de documento',
            }),
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
                <div className="relative max-w-sm flex-1">
                    ...
                    <Input
                        ...
                        className="pl-8"
                        data-test="clientes-search-input"
                    />
                </div>
                    ...
                    <TableBody>
                        {table.getRowModel().rows.length ? (
                            table.getRowModel().rows.map((row) => (
                                <TableRow key={row.id} data-test="cliente-row">
            ...
            <div className="flex items-center justify-between">
                <p className="text-muted-foreground text-sm">
                    {filteredCount > 0
                        ? `Mostrando ${pageStart}–${pageEnd} de ${filteredCount} clientes`
                        : 'Sin resultados'}
                </p>
